...
def KGIN(i):
    ...
    pd.DataFrame(drug_emb.detach().cpu()).to_csv( 'DrugCombmodel/data/KGANSynergy/DrugCombDB/CG256/embeddings_drugs_G2_'+str(i+1)+'.txt',sep='\t', header=None, index=True)
    pd.DataFrame(cell_emb.detach().cpu()).to_csv('DrugCombmodel/data/KGANSynergy/DrugCombDB/CG256/embeddings_cells_G2_'+str(i+1)+'.txt',sep='\t', header=None, index=True)
...
if __name__=='__main__':
    ...
    for i in range(1,6):
        ...
        #np.save('DrugCombmodel/splitresult/train/CG'+str(i)+'.npy',CG_pred2)
        #np.save('DrugCombmodel/splitresult/test/CG'+str(i)+'.npy',CG_pred1)
        ...
        drug_emb2 = pd.read_csv('DrugCombmodel/data/KGANSynergy/DrugCombDB/CG256/embeddings_drugs_G2_'+str(i)+'.txt',sep='\t', header=None, index_col=0)
        cell_emb2 = pd.read_csv('DrugCombmodel/data/KGANSynergy/DrugCombDB/CG256/embeddings_cells_G2_'+str(i)+'.txt',sep='\t', header=None, index_col=0)
        drug_emb2=torch.tensor(np.float32(drug_emb2[:n_drugs])).to(device)
        cell_emb2=torch.tensor(np.float32(cell_emb2)).to(device)
        
        

        all_emb1=np.loadtxt('DrugCombmodel/data/KGANSynergy/DrugCombDB/256/entity_emb_TransE_DrugCombDB_256.txt',delimiter='\t')
        drug_emb1=torch.tensor(np.float32(all_emb1[:n_drugs])).to(device)
        cell_emb1=torch.tensor(np.float32(all_emb1[n_drugs:n_drugs+76])).to(device)
        #drug_emb1=torch.tensor(np.float32(all_emb1[:21])).to(device)

        drug_smiles=np.loadtxt("DrugCombmodel/data/KGANSynergy/DrugCombDB/DrugMACCS.txt")
        drug_smiles=torch.tensor(np.float32(drug_smiles)).to(device)
        cell_express=np.loadtxt("DrugCombmodel/data/KGANSynergy/DrugCombDB/cell_expression.txt",delimiter='\t')
    ...
    #AUPR.append(AUPR1)
    #AUC.append(AUC1)
    fw1=open('DrugCombmodel/result/DrugCombDB3.1/test_result_model_CG.txt','w')
    for i in range(5):
    ...
    #fw2=open('DrugCombmodel/result/DrugCombDB3.1/test_result_model_3Glayerdim32early=5lr=1e-3epoch1000fc1.txt','w')
    #for i in range(5):
